    ...
    on<_Started>((event, emit) async {
      ...
      if (event.currentRouteName == RouteNames.komikListScreen) {
        ...
        res.fold(
          ...
          (r) => emit(state.copyWith(komikResult: r, isLoading: false)),
        );
      }
      ...
      /// Search Anoboy Data
        ...
        /// Search Anoboy Data
        final res = await getIt<AnoboyRepository>().searchAnime(
          keyword: keyword,
        );
        ...
        res.fold(
          (l) => emit(state.copyWith(isLoading: false)),
          (r) => emit(state.copyWith(anoboyResult: r, isLoading: false)),
        );
      }
    ...
    on<_LoadMore>((event, emit) async {
      if (!state.isLoadMore && !state.isLoading) {
        ...
        if (state.komikResult.nextPage != null &&
          ...
          res.fold((l) => emit(state.copyWith(isLoadMore: false)), (r) {
            /// Komik Result
            ...
            /// Emit komik result
            emit(state.copyWith(komikResult: komikResult, isLoadMore: false));
          });
          ...
          res.fold(
            ...
            (r) {
              ...
              /// Emit komik result
              emit(
                state.copyWith(anoboyResult: anoboyResult, isLoadMore: false),
              );
            },
          );
        }
