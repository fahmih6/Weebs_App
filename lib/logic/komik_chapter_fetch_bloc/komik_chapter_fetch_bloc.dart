...
part 'komik_chapter_fetch_event.dart';
part 'komik_chapter_fetch_state.dart';
part 'komik_chapter_fetch_bloc.freezed.dart';

class KomikChapterFetchBloc
    extends Bloc<KomikChapterFetchEvent, KomikChapterFetchState> {
  KomikChapterFetchBloc() : super(const _Initial()) {
    /// Started
    on<_Started>((event, emit) async {
      ...
      final chapterImages = await repo.getKomikDetailChapterImages(
        url: "$endpoint${event.param}",
      );

      /// Act according tot he result
      chapterImages.fold(
        (l) {
          emit(_Completed(errorMessage: l.message));
        },
        (r) {
          /// Last chapter list
          ...
          emit(_Completed(chapterData: lastChapterList));
        },
      );
